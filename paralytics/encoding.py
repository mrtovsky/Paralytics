...
class TargetEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        for col in self.columns:
            ...
            if self.nan_as_category and df[col].isnull().sum() > 0:
                nan_val = df[df[col].isnull()].target.agg(self._penalized_mean,
                                                          y=y,
                                                          alpha=self.alpha)
    ...
    def transform(self, X, y=None):
        ...
        if y is None or self.cv is None:
            ...
            X_new[self.columns] = self._transform_train_cv(X_new[self.columns], 
                                                           y)
    ...
    def _transform_train_cv(self, X, y):
        ...
        By using only the part of the data (k-1 folds) it estimates the encoding
        value for the leftover fold. It performs those activities independently 
        for each category. As a result we get synthetic values located more 
        dense in a given space. 
        ...
        for train_idx, encode_idx in kf.split(df):
            inner_kf = KFold(n_splits=self.inner_cv, shuffle=self.shuffle,
                             random_state=self.random_state)
           
            # Declaring list of DataFrames that will store inner mean values.
            mean_df_list = [pd.DataFrame({f'{col}': X[col].unique()}) 
                            for col in X.columns]
            inner_df = df.iloc[train_idx].copy()
            for loop_idx, (inner_train_idx, _) in \
              enumerate(inner_kf.split(inner_df)):
                for idx, col in enumerate(X.columns):
                    agg_values = pd.DataFrame(inner_df.iloc[inner_train_idx]\
                                 .groupby(col).target\
                                 .agg('mean'))\
                                 .reset_index()
                    mean_df_list[idx] = mean_df_list[idx]\
                                        .merge(
                                               agg_values, 
                                               how='left', 
                                               on=f'{col}',  
                                               suffixes=(f'_{loop_idx-1}', 
                                                         f'_{loop_idx}')
                                               )
            mean_df_list = [data.set_index(data.columns[0])\
                            .agg(self._penalized_mean,
                                 axis=1, 
                                 n_instances=X.iloc[train_idx, col_idx].count(), 
                                 y=y[train_idx],
                                 alpha=self.alpha * (1 - 1/self.cv)).to_dict()
                            for col_idx, data in enumerate(mean_df_list)]
            
            for loop_idx, col in enumerate(X_new.columns):
                X_new[col].iloc[encode_idx] = X[col].map(mean_df_list[loop_idx])
